...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  if (!avatarLocalPath) {
    throw new ApiError(400, "Avatar image to upload is missing");
  }

  const avatar = await uploadOnCloudinary(avatarLocalPath);
  if (!avatar.url) {
    throw new ApiError(500, "Failed To Upload Image On Cloudinary Server");
  }
  const user = await User.findByIdAndUpdate(
    req.user?._id,
    {
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImageLocalPath) {
    throw new ApiError(400, "cover image to upload is missing");
  }
